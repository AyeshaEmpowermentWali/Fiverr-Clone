...
try {
    ...
    if (!empty($search)) {
        $where_conditions[] = "(g.title LIKE ? OR g.description LIKE ? OR g.tags LIKE ?)";
        $search_term = "%{$search}%";
        $params[] = $search_term;
        $params[] = $search_term;
        $params[] = $search_term;
    ...
    if ($min_price > 0) {
        $where_conditions[] = "g.basic_price >= ?";
        $params[] = $min_price;
    }
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE {$where_clause}
        ORDER BY COALESCE(g.is_featured, 0) DESC, {$order_by}
        LIMIT ? OFFSET ?
    ");
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM gigs g JOIN users u ON g.user_id = u.id JOIN categories c ON g.category_id = c.id WHERE {$where_clause}");
...
function buildFilterUrl($params) {
    ...
    return 'browse.php?' . http_build_query($current_params);
}
...
<head>
    ...
    <title>Browse Services - FiverrClone</title>
        ...
        <div class="container">
            <div class="results-header">
                <h2 class="results-count">
                    <?= number_format($total_gigs) ?> services found
                    <?php if (!empty($search)): ?>
            ...
            <?php if (empty($gigs)): ?>
                <div class="empty-state">
                    ...
                    <br>
                    <a href="categories.php" style="color: #1dbf73; text-decoration: none; font-weight: 600;">
                        ...
                        <div class="gig-content">
                            ...
                            <div class="gig-footer">
                                ...
                                <div class="gig-category">
                                    <?= htmlspecialchars($gig['category_name']) ?>
                                </div>
                            </div>
                        </div>
                    </div>
